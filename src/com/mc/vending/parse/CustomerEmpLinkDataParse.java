...
public class CustomerEmpLinkDataParse implements DataParseListener {
	...
	public void requestCustomerEmpLinkData(String optType, String requestURL, String vendingId) {
		...
		try {
			...
			e.printStackTrace();
			ZillionLog.e(this.getClass().toString(), "======>>>>>客户员工网络请求数据异常!");
		}
	}
	...
	public void parseJson(BaseData baseData) {
		...
		List<CustomerEmpLinkData> addList = new ArrayList<CustomerEmpLinkData>();
		List<CustomerEmpLinkData> updateList = new ArrayList<CustomerEmpLinkData>();
		List<CustomerEmpLinkData> deleteList = new ArrayList<CustomerEmpLinkData>();
		for (CustomerEmpLinkData customerEmpLinkData : list) {
			if (customerEmpLinkData.getCRUD().equals("D")) {
				deleteList.add(customerEmpLinkData);
			} else if (customerEmpLinkData.getCRUD().equals("C")) {
				addList.add(customerEmpLinkData);
			} else if (customerEmpLinkData.getCRUD().equals("U")) {
				updateList.add(customerEmpLinkData);
			}
		}
		CustomerEmpLinkDbOper customerEmpLinkDbOper = new CustomerEmpLinkDbOper();
		if (!addList.isEmpty()) {
			boolean addFlag = customerEmpLinkDbOper.batchAddCustomerEmpLink(addList);
			if (addFlag) {
		...
		if (!deleteList.isEmpty()) {
			boolean deleteFlag = customerEmpLinkDbOper.batchDeleteCustomerEmpLink(deleteList);
			if (deleteFlag) {
				Log.i("[customerEmpLink]:", "卡/密码批量删除成功!" + "======" + list.size());
				DataParseHelper parseHelper = new DataParseHelper(this);
				parseHelper.sendLogVersion(list.get(0).getLogVersion());
			} else {
				ZillionLog.e("[customerEmpLink]:", "客户员工批量删除失败!");
			}
		}
		if (!updateList.isEmpty()) {
			boolean deleteFlag = customerEmpLinkDbOper.batchDeleteCustomerEmpLink(updateList);
			boolean addFlag = customerEmpLinkDbOper.batchAddCustomerEmpLink(updateList);
			if (deleteFlag && addFlag) {
				Log.i("[customerEmpLink]:", "客户员工批量修改成功!" + "======" + list.size());
				DataParseHelper parseHelper = new DataParseHelper(this);
				parseHelper.sendLogVersion(list.get(0).getLogVersion());
			} else {
				ZillionLog.e("[customerEmpLink]:", "客户员工批量修改失败!");
			}
		}
		if (this.listener != null) {
			this.listener.parseRequestFinised(baseData);
	...
	public List<CustomerEmpLinkData> parse(JSONArray jsonArray) {
		...
		try {
			...
			e.printStackTrace();
			ZillionLog.e(this.getClass().toString(), "======>>>>>客户员工解析数据异常!");
		}
		return list;
	}
