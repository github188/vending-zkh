...
public class ProductMaterialPowerDbOper {
     ...
     * @param cusId 客户ID
     * @param vc2Id 售货机卡/密码权限.ID
     * @param vp1Id 售货机产品.ID
     * @return
     */
	 ...
	 * @param list
	 */
	public boolean batchDeleteProductMaterialPower(List<ProductMaterialPowerData> list) {
		boolean flag = false;
		SQLiteDatabase db = AssetsDatabaseManager.getManager().getDatabase();
		try {
			// 开启事务
			...
			for (ProductMaterialPowerData productMaterialPowerData : list) {
				db.delete("ProductMaterialPower", "PM1_ID=?", new String[] { productMaterialPowerData.getPm1Id() });
			}
			// 数据成功，设置事物成功标志
			db.setTransactionSuccessful();
			// 保存数据
