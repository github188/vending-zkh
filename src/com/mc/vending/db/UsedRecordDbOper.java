...
/**
 * 卡与产品领用　操作类
 *  <table border="0" cellpadding="0" cellspacing="0" width="1309" style="width:983pt;" class="ke-zeroborder"> 
   <tbody> 
    <tr> 
...
public class UsedRecordDbOper {
    ...
    public int getTransQtyCount(String cardId, String skuId, String dateStr) {
        ...
        Cursor c = db
                .rawQuery(
                        "SELECT max(UR1_Quantity) as UR1_Quantity FROM UsedRecord WHERE UR1_CD1_ID=? and UR1_PD1_ID =? and ModifyTime >=?",
                        new String[] { cardId, skuId ,dateStr});
        while (c.moveToNext()) {
    ...
    String insertSql = "insert into UsedRecord(UR1_ID,UR1_M02_ID,UR1_CD1_ID,UR1_VD1_ID,UR1_PD1_ID,UR1_Quantity,"
                             + "CreateUser,CreateTime, ModifyUser,ModifyTime,RowVersion)"
                             + "values(?,?,?,?,?,?,?,?,?,?,?)";
