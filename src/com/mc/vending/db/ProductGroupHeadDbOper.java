...
public class ProductGroupHeadDbOper {
	 ...
	 * @param list
	 */
	public boolean batchDeleteProductGroupHead(List<ProductGroupHeadData> list) {
		boolean flag = false;
		SQLiteDatabase db = AssetsDatabaseManager.getManager().getDatabase();
		try {
			// 开启事务
			...
			for (ProductGroupHeadData productGroupHeadData : list) {
				db.delete("ProductGroupHead", "PG1_ID=?", new String[] { productGroupHeadData.getPg1Id() });
				db.delete("ProductGroupDetail", "PG2_PG1_ID=?", new String[] { productGroupHeadData.getPg1Id() });
			}
			// 数据成功，设置事物成功标志
			db.setTransactionSuccessful();
			// 保存数据
	 ...
	 * @param list
	 */
	public boolean batchDeleteProductGroupHeadOnly(List<ProductGroupHeadData> list) {
		boolean flag = false;
		SQLiteDatabase db = AssetsDatabaseManager.getManager().getDatabase();
		try {
			// 开启事务
     ...
     * @param list
     */
    public boolean batchAddProductGroupHeadOnly(List<ProductGroupHeadData> list) {
        boolean flag = false;
        String headSql = "insert into ProductGroupHead(PG1_ID,PG1_M02_ID,PG1_CU1_ID,PG1_CODE,PG1_Name,PG1_CreateUser,PG1_CreateTime,PG1_ModifyUser,PG1_ModifyTime,PG1_RowVersion)"
                         + "values(?,?,?,?,?,?,?,?,?,?)";
        SQLiteDatabase db = AssetsDatabaseManager.getManager().getDatabase();
     ...
     * @param list
     */
    public boolean batchAddProductGroupDetailOnly(List<ProductGroupDetailData> list) {
        boolean flag = false;
        String detailSql = "insert into ProductGroupDetail(PG2_ID,PG2_M02_ID,PG2_PG1_ID,PG2_PD1_ID,PG2_GroupQty,PG2_CreateUser,PG2_CreateTime,PG2_ModifyUser,PG2_ModifyTime,PG2_RowVersion)"
                + "values(?,?,?,?,?,?,?,?,?,?)";
        SQLiteDatabase db = AssetsDatabaseManager.getManager().getDatabase();
	 ...
	 * @param list
	 */
	public boolean batchDeleteProductGroupDetailOnly(List<ProductGroupDetailData> list) {
		boolean flag = false;
		SQLiteDatabase db = AssetsDatabaseManager.getManager().getDatabase();
		try {
			// 开启事务
			db.beginTransaction();
			for (ProductGroupDetailData productGroupDetailData : list) {
				db.delete("ProductGroupDetail", "PG2_ID=?", new String[] { productGroupDetailData.getPg2Id() });
			}
			// 数据成功，设置事物成功标志
			db.setTransactionSuccessful();
			// 保存数据
