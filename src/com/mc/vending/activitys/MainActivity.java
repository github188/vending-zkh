...
public class MainActivity extends Activity implements DataParseRequestListener {
	...
	private void requestPwd() {
		...
		parse.requestAutherData(Constant.HTTP_OPERATE_TYPE_DESGET, Constant.METHOD_WSID_AUTHER, Tools.getMacAddress());
	}
	...
	public void requestVendingExistParse() {
		...
		parse.setListener(this);
		parse.requestVendingData(Constant.HTTP_OPERATE_TYPE_GETDATA, Constant.METHOD_WSID_VENDING, vendCode, isInit);
		isInit = false;
	}
	...
	public void parseRequestFailure(BaseData baseData) {
		...
		if (Constant.METHOD_WSID_VERSION.equals(baseData.getRequestURL())) {
			...
			// 0) {
			Object userObj = baseData.getUserObject();
			if (userObj == null) {
				resetAlertMsg(baseData.getReturnMessage());
				return;
			}
			boolean existFlag = (Boolean) userObj;
			if (!existFlag) {
				resetAlertMsg("售货机编号不正确，请联系系统管理员");
				return;
			}
