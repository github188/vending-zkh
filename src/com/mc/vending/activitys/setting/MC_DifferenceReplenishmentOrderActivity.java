...
public class MC_DifferenceReplenishmentOrderActivity extends BaseActivity {
                                ...
                                public void handleMessage(Message msg) {
                                    switch (msg.what) {
                                        case MC_SettingActivity.SET_SUCCESS:
                                            if (dataList.size() > 0) {
                                                adapter = new MC_DifferenceReplenishmentOrderAdapter(
                                                    MC_DifferenceReplenishmentOrderActivity.this,
                                                    dataList, listView);
                                                listView.setAdapter(adapter);
                                            } else {
                                                resetAlertMsg("没有补货单，请联系管理员或先进行一键补货！");
                                            }
                                            stopLoading();
                                            break;
                                        case MC_SettingActivity.SET_SELECT:
                                            goDifferenceReplenishmentActivity(Integer
    ...
    */
    ...
    private void goDifferenceReplenishmentActivity(int position) {
        ...
        intent.setClass(MC_DifferenceReplenishmentOrderActivity.this,
            MC_DifferenceReplenishmentActivity.class);
