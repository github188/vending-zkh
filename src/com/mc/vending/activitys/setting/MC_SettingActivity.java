 ...
 *
 */
public class MC_SettingActivity extends BaseActivity
	...
	private void initObject() {
		...
		if (Integer.valueOf(Constant.HEADER_VALUE_CLIENTVER.replace(".", "")) >= Constant.VERSION_STOCK_SYNC) {
			...
			tv_public_title.setText(getResources().getString(R.string.set_vending_number) + vendData.getVd1Code());
	...
	private void setReplenishment() {
		...
		Thread downLoadData = new Thread(new Runnable() {
			...
			public void run() {
				...
				ServiceResult<Boolean> result = ReplenishmentService.getInstance().oneKeyReplenishment(wrapperData);
				if (!result.isSuccess()) {
	...
	public void parseRequestFinised(BaseData baseData) {
		...
		if (Constant.METHOD_WSID_CHECK_INIT.equals(baseData.getRequestURL())) {
			Dialog dialog = new AlertDialog.Builder(this).setMessage(getString(R.string.alert_init_msg))
