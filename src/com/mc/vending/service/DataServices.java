...
public class DataServices extends Service implements DataParseRequestListener, Serializable {
	...
	// private String sql201 = "if NOT exists(select * from syscolumns where
	// id=object_id('Vending') and name='VD1_CardType') "
	// + "begin ALTER TABLE Vending add COLUMN VD1_CardType varchar(10) not null
	// DEFAULT '1' "
	// + "end;";
	// private String sql210 = "if NOT exists(select * from syscolumns where
	// id=object_id('Card') and name='CD1_ProductPower')"
	// + "begin ALTER TABLE Card add COLUMN CD1_ProductPower varchar(1) not null
	// DEFAULT '0' "
	// + "end;";
	...
	public void onCreate() {
		...
		// if (Integer.valueOf(Constant.HEADER_VALUE_CLIENTVER.replace(".", ""))
		// >= 201) {
	...
	private void startConfigTimer() {
		...
		// configTimer.schedule(configTask, configTimerLength,
		// configTimerLength);
	...
	private void startUploadTimer() {
		...
		if (replenishmentStatusTimerLength > 0) {
			...
			replenishmentStatusTimer.schedule(replenishmentStatusTask, replenishmentStatusTimerLength,
					replenishmentStatusTimerLength);
		}
		// 补货差异 定时任务 全表 上传数据
		if (replenishmentDiffTimerLength > 0) {

			replenishmentDiffTimer.schedule(replenishmentDiffTask, replenishmentDiffTimerLength,
					replenishmentDiffTimerLength);
		}
		// 盘点记录 定时任务 全表 上传数据
		...
		if (stockTransactionTimerLength > 0) {
			...
			stockTransactionTimer.schedule(stockTransactionTask, stockTransactionTimerLength,
					stockTransactionTimerLength);
		}
		/********************************************************************
		 * 数据上传接口 end
		 ****************************************************/
	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case MESSAGE_CONFIG:
				// 同步下载接口配置数据
				// ZillionLog.i(this.getClass().getSimpleName(),
				// "MESSAGE_CONFIG");
				requestConfigParse();
				break;
			...
			case MESSAGE_STOCKTRANSACTION:
				...
				if (!StockTransactionDataParse.getInstance().isSync) { // 没有上传
					...
					StockTransactionDataParse.getInstance().requestStockTransactionData(
							Constant.HTTP_OPERATE_TYPE_INSERT, Constant.METHOD_WSID_STOCKTRANSACTION, vendingId);
				}
				break;
			default:
				break;
