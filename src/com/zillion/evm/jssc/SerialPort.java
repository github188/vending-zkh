...
public class SerialPort {
    ...
    public boolean openPort() throws SerialPortException {
        ...
        if (portName != null) {
            boolean useTIOCEXCL = (System.getProperty(SerialNativeInterface.PROPERTY_JSSC_NO_TIOCEXCL) == null && System
                    .getProperty(SerialNativeInterface.PROPERTY_JSSC_NO_TIOCEXCL.toLowerCase()) == null);
            try {
        ...
        if (portHandle == SerialNativeInterface.ERR_PORT_BUSY) {
            ...
            throw new SerialPortException(portName, "openPort()",
                    SerialPortException.TYPE_INCORRECT_SERIAL_PORT);
        }
        portOpened = true;
        return true;
    }
     ...
     * @since 0.8
     */
    public boolean setParams(int baudRate, int dataBits, int stopBits, int parity, boolean setRTS,
     ...
     * @since 2.8.0
     */
    public boolean writeString(String string, String charsetName) throws SerialPortException,
    ...
    /**
     * Read Hex string from port (example: FF 0A FF). Separator by default is a
     * space
     *
     * @param byteCount
     *            count of bytes for reading
    ...
    private void waitBytesWithTimeout(String methodName, int byteCount, int timeout)
     ...
     * @since 2.0
     */
    public byte[] readBytes(int byteCount, int timeout) throws SerialPortException,
     ...
     * @since 2.0
     */
    public String readString(int byteCount, int timeout) throws SerialPortException,
            SerialPortTimeoutException {
        checkPortOpened("readString()");
     ...
     * @since 2.0
     */
    public String readHexString(int byteCount, int timeout) throws SerialPortException,
            SerialPortTimeoutException {
        checkPortOpened("readHexString()");
     ...
     * @since 2.0
     */
    public String readHexString(int byteCount, String separator, int timeout) throws SerialPortException,
            SerialPortTimeoutException {
        checkPortOpened("readHexString()");
        waitBytesWithTimeout("readHexString()", byteCount, timeout);
     ...
     * @since 2.0
     */
    public String[] readHexStringArray(int byteCount, int timeout) throws SerialPortException,
            SerialPortTimeoutException {
        checkPortOpened("readHexStringArray()");
     ...
     * @since 2.0
     */
    public int[] readIntArray(int byteCount, int timeout) throws SerialPortException,
            SerialPortTimeoutException {
        checkPortOpened("readIntArray()");
    ...
    /**
     * Internal method. Add event listener. Object of
     * <b>"SerialPortEventListener"</b> type shall be sent to the method. This
     * object shall be properly described, as it will be in charge for handling
     * of occurred events. Also events mask shall be sent to this method, to do
     * it use variables with prefix <b>"MASK_"</b> for example
     * <b>"MASK_RXCHAR"</b>. If <b>overwriteMask == true</b> and mask has been
     * already assigned it value will be rewrited by <b>mask</b> value, if
     * <b>overwriteMask == false</b> and mask has been already assigned the new
     * <b>mask</b> value will be ignored, if there is no assigned mask to this
     * serial port the <b>mask</b> value will be used for setting it up in spite
     * of <b>overwriteMask</b> value
     *
     * @see #setEventsMask(int) setEventsMask(int mask)
     *
     * @throws SerialPortException
     */
    private void addEventListener(SerialPortEventListener listener, int mask, boolean overwriteMask)
            throws SerialPortException {
        ...
        if (!eventListenerAdded) {
            ...
            //since 2.2.0 ->
            try {
                Method method = eventListener.getClass().getMethod("errorOccurred",
                        new Class[] { SerialPortException.class });
            ...
            while (!threadTerminated) {
                ...
                for (int i = 0; i < eventArray.length; i++) {
                    if (eventArray[i][0] > 0 && !threadTerminated) {
                        eventListener.serialEvent(new SerialPortEvent(portName, eventArray[i][0],
                                eventArray[i][1]));
                        //FIXME
                        /*
                         * if(methodErrorOccurred != null){ try {
                         * methodErrorOccurred.invoke(eventListener, new
                         * Object[]{new SerialPortException("port", "method",
            ...
            while (!super.threadTerminated) {
                ...
                for (int i = 0; i < eventArray.length; i++) {
                    ...
                    if (eventType > 0 && !super.threadTerminated) {
                         ...
                         */
                        case MASK_TXEMPTY:
                            if (((mask & MASK_TXEMPTY) == MASK_TXEMPTY) && (eventValue == 0)
                                    && interruptTxChanged) {
                                sendEvent = true;
                            }
                            break;
